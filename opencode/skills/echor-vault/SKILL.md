...
```yaml
type: service              # service | personal-project | component (required)
status: active              # active | paused | archived (required)
repo: https://github.com/... | local path   # required
...
relates_to: ["[[lucyna]]"] # wikilinks, no strings sueltos (required, may be empty list)
components: [ingest-worker]  # required, may be empty list
part_of: null               # only on type: component (optional otherwise, null)
...
- `relates_to` entries MUST be Obsidian wikilinks `[[slug]]`. A plain string
  does not create a graph edge and is invalid.
...
- Who relates to X: `rg -l "\[\[X\]\]" ~/dev/echor/projects`
