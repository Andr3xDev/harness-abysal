---
description: Manage the Echor vault — onboard, update, ask, or validate project knowledge
---
...
3. `ask <question>` → delegate to echor-consultador with the question, read-only
4. `validate [slug|all]` → delegate to echor-validator with the target slug or "all", read-only audit
...
Only onboard and update write to the vault; ask and validate never modify it.
