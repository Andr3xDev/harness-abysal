/**
 * Engram — OpenCode plugin adapter (V2)
 *
...
// ENGRAM_BIN override first, then the V1 Bun.which→PATH→/usr/bin fallback chain,
// duplicated with node:child_process since Bun.which is Bun-only.
function resolveEngramBin(): string {
  const env = process.env.ENGRAM_BIN?.trim()
  if (env) return env
  try {
    const res = spawnSync("which", ["engram"], { encoding: "utf8" })
...
// The V1 Task tool was renamed to `subagent` in V2; match both so the
// passive capture keeps working regardless of which name the runtime reports.
...
export default Plugin.define({
  ...
  async setup(ctx) {
    ...
    // Track sub-agent session IDs so we can suppress their hook registrations.
    // Sub-agents have a parentID or a title ending in " subagent)".
    ...
    if (!running) {
      try {
        const child = spawn(resolveEngramBin(), ["serve"], {
          stdio: "ignore",
          detached: true,
        })
        child.unref()
    ...
    try {
      ...
      if (fs.existsSync(manifestFile)) {
        const child = spawn(resolveEngramBin(), ["sync", "--import"], {
          cwd: directory,
          stdio: "ignore",
          detached: true,
        })
    ...
    // ─── Event Listeners ─────────────────────────────────────────────────
    // V2 events are flat: { type, data: { sessionID, parentID?, title?, ... } }.
    // The V1 payload nested them under event.properties.info.
      ...
      try {
        for await (const event of ctx.event.subscribe({ signal: eventController.signal })) {
          ...
          if (event.type === "session.created") {
            ...
            // Sub-agent sessions must NOT be registered as top-level Engram
            // sessions. They cause massive session inflation
    ...
    // ─── User Prompt Capture ──────────────────────────────────────────────
    // The V2 prompt hook runs once at prompt admission, before the LLM sees it
    // (same once-per-message semantics as the V1 chat.message hook).
    ...
    // Passive capture: when a subagent/Task tool completes, POST its output
    // to the passive capture endpoint so the server extracts learnings.
    ...
    ctx.tool.hook("execute.after", (event) => {
      ...
      // Passive capture: extract learnings from sub-agent tool output
      if (SUBAGENT_TOOLS.has(event.tool.toLowerCase()) && event.status === "completed" && sessionId) {
        ...
        if (text.length > 50) {
          void engramFetch("/observations/passive", {
            method: "POST",
            body: {
              session_id: sessionId,
    ...
    // Injects MEMORY_INSTRUCTIONS into the system prompt of every agent-loop
    // request and compaction request. This ensures the agent ALWAYS knows
    // about Engram, even after compaction.
    //
    ...
    const appendInstructions = (event: { system: Array<{ type: string; text: string }> }) => {
      if (event.system.length > 0) {
        const last = event.system[event.system.length - 1]
        last.text += "\n\n" + MEMORY_INSTRUCTIONS
      } else {
        event.system.push(SystemPart.make(MEMORY_INSTRUCTIONS))
      }
        ...
        try {
          ...
          try {
            const sessionRes = await fetch(
              `${ENGRAM_URL}/sessions/${encodeURIComponent(sessionID)}`,
          ...
          const nudge =
            ...
            "call mem_save now."
          if (event.system.length > 0) {
            event.system[event.system.length - 1].text += nudge
          } else {
            event.system.push(SystemPart.make(nudge))
          }
    ...
    ctx.session.hook("context", appendInstructions)
    ctx.session.hook("compaction", appendInstructions)
    ...
    // 2. Inject context from previous sessions into the compaction transcript
    // 3. Tell the compressor to remind the new agent to save memories
    //
    // Mapping note: V1 pushed strings onto output.context. The V2 compaction
    // event carries `messages` (the transcript being summarized) and OpenCode
    // appends its summary prompt AFTER hooks run — so we push operator-authored
    // system messages onto event.messages, which land in the compressor's
    // transcript right before the summary prompt.
    ...
    ctx.session.hook("compaction", async (event) => {
      ...
      if (data?.context) {
        event.messages.push(Message.system(data.context))
      }
    ...
    // Cleanup: stop the event subscription if the plugin unloads.
