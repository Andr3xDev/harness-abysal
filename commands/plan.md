...
6. Read the per-change `.openspec.yaml` before spec dispatch. If `skip_specs: true` (valid only when the change has no observable behavior change), skip sdd-spec and use proposal/design acceptance scenarios; otherwise delegate to sdd-spec. Then delegate to sdd-design → sdd-tasks
